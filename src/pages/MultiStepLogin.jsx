...
const MultiStepLogin = () => {
  const [step, setStep] = useState('email'); // 'email', 'phone', 'otp', 'register'
  ...
  const handlePhoneVerify = async () => {
    ...
    try {
      ...
      const result = await firebaseOTPService.sendOTPToPhone(formattedPhone);
  ...
  const handleVerifyOTP = async (otp) => {
    ...
    try {
      ...
      if (currentOTPType === 'phone') {
        // Use Firebase OTP service for phone verification
        console.log('🔥 Using Firebase OTP verification for phone');
        result = await firebaseOTPService.verifyOTP(otp);
        ...
        // Use backend OTP service for email verification
        console.log('📧 Using backend OTP verification for email');
        result = await otpService.verifyOTP(email, otp, 'email', 'verification');
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      if (currentOTPType === 'phone') {
        result = await firebaseOTPService.sendOTPToPhone(phone);
      } else {
        result = await otpService.sendOTP(email, 'email', 'verification');
      }
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 overflow-hidden">
            ...
            <div className="mb-6 space-y-3">
              ...
              <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                ...
                <span className="text-xs text-green-600">+91 {phone}</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-[#53755d] text-white py-3 px-4 rounded-lg font-semibold hover:bg-[#3e5d49] disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center"
              >
                {isLoading ? (
                  <>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 overflow-hidden">
          ...
          <div className="flex justify-center mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div className={`w-12 h-1 ${phoneVerified ? 'bg-green-500' : 'bg-gray-300'}`}></div>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                step === 'register' ? 'bg-[#53755d] text-white' : 'bg-gray-300 text-gray-600'
              }`}>
                3
              </div>
            </div>
